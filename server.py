    ...
    while True:
        try:
            ...
            if notAutoFunc == False:
                ...
                for iteration in range(iterations):
                    ...
                    if (iteration % 100) == 9:
                        ...
                        # 当前的计算状态，格式为{"status": "busy", "iteration": <当前迭代轮次>, "R2":<当前迭代R2> }
                ...
                # 计算结束后的状态，格式为{"status": "done", "R2": <计算最终的R2>, "Model": <计算得到的RGB算式模型>}
            ...
            if methord == 1:
                ...
                # 直线拟合结果，格式为{"status": "success", "result": <直线拟合的结果参数，格式为[斜率,截距,R2]>, "points": <用于绘图的数据点，格式为[[x0,y0,y_err0],[x1,y1,y_err1],[...]]>}
...
async def process_image_sample(websocket: WebSocket):
    while True:
        try:
            ...
            # 样品计算状态，格式为{"status": "done", "result": [计算值，计算值误差]}
            await websocket.close()
        except Exception as e:
            break
